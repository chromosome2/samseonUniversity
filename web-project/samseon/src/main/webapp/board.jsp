...
<% Map articleMap=(Map)request.getAttribute("articleMap"); int currentPage=Integer.parseInt(articleMap.get("pageNum").toString()); %>
                ...
                <c:forEach var="article" items="${articleList}" varStatus="articleNum">
                  <tr>
                    <td>${articleNum.count}</td>
                      <td><a href="${contextPath}/board/viewArticle.do?articleNo=${article.articleNo}">${article.title}</a>
                      </td>
              ...
              <c:if test="${cnt != 0}">
                ...
                <c:forEach var="page" begin="${startPage}" end="${endPage}" step="1">
                  ...
                  <c:if test="${page == currentPage}">
                    <li class="paging_num paging_active"><a class="num_active" href="${contextPath}/board/listArticles.do?section=${section}&pageNum=${page}&search=${articleMap.searchKey}">${page}</a>
                    </li>
                  </c:if>

                  <c:if test="${page != currentPage}">
                    <li class="paging_num"><a href="${contextPath}/board/listArticles.do?section=${section}&pageNum=${page}&search=${articleMap.searchKey}">${page}</a>
                    </li>
                  </c:if>
                </c:forEach>

                <c:if test="${endPage < pageCount}">
                  <li class="paging_last"><a class="paging_arrow" href="${contextPath}/board/listArticles.do?section=${section+1}&pageNum=${startPage+pageBlock}&search=${articleMap.searchKey}">>></a>
                  </li>
                </c:if>
              </c:if>
